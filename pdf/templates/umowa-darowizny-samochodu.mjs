...
export async function buildUmowaDarowiznySamochoduPdf(outputPath = path.resolve("public/wzory/umowa-darowizny-samochodu-wzor.pdf")) {
  const meta = createDocumentMeta({
    title: "Umowa darowizny samochodu",
    subtitle: "Wzór przekazania pojazdu w formie darowizny",
    documentType: "UDS",
    sourceUrl: "https://oc.documenty.pl/dokumenty/umowa-darowizny-samochodu",
    revision: "r1",
  });
  ...
  sectionHeader(1, "Darczyńca", 10, 78.2, 92);
  ...
  sectionHeader(2, "Obdarowany", 108, 78.2, 92);
  ...
  sectionHeader(3, "Przedmiot darowizny — pojazd", 10, 145, 92);
  ...
  field("Rok produkcji", 15, 187, 82, 7);

  sectionHeader(4, "Wartość i stan pojazdu", 108, 145, 92);
  layout.box(108, 156.8, 92, 56.5);
  field("Wartość rynkowa pojazdu (dla celów podatkowych)", 113, 164, 82, 7, true);
  field("Stan techniczny w dniu darowizny", 113, 175.5, 82, 7, true);
  ...
  layout.text("Darczyńca oświadcza, że jest właścicielem pojazdu i przekazuje go nieodpłatnie Obdarowanemu.", 16, 236.5, typography.body, theme.colors.text);
  layout.text("Obdarowany oświadcza, że darowiznę przyjmuje i zapoznał się ze stanem technicznym pojazdu.", 16, 242.1, typography.body, theme.colors.text);
  layout.text("Podpis Darczyńcy", 16, 250, typography.h2, theme.colors.primary, { bold: true });
  layout.line(16, 260, 70, 260, theme.colors.primary, 0.3);
  layout.text("Podpis Obdarowanego", 110, 250, typography.h2, theme.colors.primary, { bold: true });
